...
func GetListVehicleYears() ([]int16, error) {
	...
	err = db.Select(&listVehicleYear, "SELECT DISTINCT vehicle_year FROM vehicle ORDER BY vehicle_year DESC")
...
func GetListVehicleMakeCoverage(makeId string) (map[string][]int16, error) {
	...
	rows, err := db.Query("SELECT DISTINCT name, vehicle_year FROM vehicle LEFT JOIN vehicle_model ON vehicle.vehicle_model_id = vehicle_model.vehicle_model_id WHERE vehicle.vehicle_make_id = ? ORDER BY vehicle_year DESC", makeId)
